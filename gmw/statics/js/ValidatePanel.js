class ValidatePanel extends React.Component {
    ...
    createProject(e, pdate) {
        ...
        if (this.state.region == 1) {
            ...
            var selectedarr = [...document.getElementById("selectProjRegions").options]
                .filter(x => x.selected)
                .map(x => "mun_" + x.value);
    ...
    closeProject(e, pdate, pid, i) {
        ...
        var template =
            "Are you sure you want to close the project for %pdate? Closing the project means you will no longer be able to validate the points unless you set up another project.";
    ...
    render() {
        if (!USER_STATE) {
            ...
            if (this.state.projects.length == 0) {
                var context = <div>You don't have any active projects at the moment.</div>;
            } else {
                var lilist = [];
                ...
                var context = (
                    <div>
                        ...
                        <ul className="sub-list">{lilist}</ul>
                    </div>
                );
                var context = (
