...
class SubscribedList extends React.Component {
    ...
    componentDidMount() {
        ...
        this.map.on("load", e => {
            this.map.bindDeeper = e => {
                this.goDeeper(e);
            };
            this.map.bindShallower = e => {
                this.goShallower(e);
            };
        ...
        if (this.currentlevel < 2) {
            if (this.selectedfeature) {
                ...
                this.map.setFeatureState(
                    {source: "activelayer", id: this.selectedfeature.id},
                    {clicked: false}
                );
            ...
            ul.push(
                <li key={i}>
                    ...
                    <input
                        ...
                        className="del-btn"
                        title="delete"
                        onClick={e => this.delSubs(e, list[i])}
                    />
                </li>
            );
        }
    ...
    getFeatures(level, region) {
        fetch(this.URLS.getfeats + "?level=" + level + "&focus=" + region)
            .then(res => res.json())
            .then(
                result => {
                    this.selectedfeature = "";
    ...
    delSubs(e, data) {
        fetch(this.URLS.delsubs + "?region=" + data + "&level=" + this.currentlevel)
            .then(res => res.json())
            .then(
                result => {
    ...
    addSubs(e) {
        var region = document.getElementById("regionBox").value;
        if (region != "") {
            fetch(this.URLS.addsubs + "?region=" + region + "&level=" + this.currentlevel)
                .then(res => res.json())
                .then(
                    result => {
